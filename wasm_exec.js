...
(() => {
	...
	if (!global.fs && global.require) {
		try { global.fs = require("fs"); } catch (e) {} // cf-workers runtime screams
	}
	...
	if (!global.performance) {
		...
		if (process.hrtime !== undefined) {
			global.performance.now = () => {
				const [sec, nsec] = process.hrtime(); return sec * 1000 + nsec / 1000000;
			};
		} else {
			const startEp = new Date().getTime();
			global.performance.now = () => { return new Date() - startEp; };
		}
	}
