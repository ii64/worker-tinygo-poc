...
module.exports = {
  ...
  plugins: [
    {
      apply: compiler => {
        compiler.hooks.compilation.tap('tinygo-build', compilation => {
          let result = spawn('node', ['build.js'], { stdio: 'inherit' })

          if (result.status != 0) {
            compilation.errors.push('tinygo build failed')
          } else {
            console.log('tinygo build complete')
          }
        })
      },
    },
    new CopyPlugin({
      patterns: [
        ...
        // correctly.
        { from: './build/main.wasm', to: './worker/module.wasm' }
      ],
    }),
